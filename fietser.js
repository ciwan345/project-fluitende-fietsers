...
document.addEventListener("DOMContentLoaded", () => { 
    ...
    if (!totaalElement || productCards.length === 0) return;
    ...
    function updateTotaal() { 
        ...
        productCards.forEach(card => { 
            ...
            const prijsElement = card.querySelector("p");
            const inputElement = card.querySelector("input");

            if (!prijsElement || !inputElement) return;

            const prijsText = prijsElement.textContent;  
            const prijs = Number(prijsText.replace(/[^0-9]/g, ""));  
            const aantal = Number(inputElement.value); 
